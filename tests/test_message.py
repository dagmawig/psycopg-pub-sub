...
async def test_async_fetch():
    ...
    # fetch message
    result = await fetch(connection_str=os.environ.get('CONNECTION_URL'), channel='sports', start_date='2024-09-06', end_date='2024-09-07', game='NFL')
...
async def test_async_publish_sub():
    ...
    sub_result = await subscribe(connection_str=os.environ.get('CONNECTION_URL'), channel='news')

    # publish message
    pub_result = await publish(connection_str=os.environ.get('CONNECTION_URL'), channel='news', data={'topic': 'business'})
